...
if (user.id === user2.id) {
      return interaction.editReply({content: `**You Can't pay to yourself**`, ephemeral: true});
}
    if (user.id === user2.bot) {
      return interaction.editReply({content: `**You Can't pay to a bot**`, ephemeral: true});
    }

  if (balance < ammount) {
      return interaction.editReply({content: `**You don't have that much money**`, ephemeral: true});
  }
